...
void Cola::mostrarATR(int posX, int posY)
{
	...
	int sSig = TC->TextWidth("0x00sig000");
	...
	strIni = "INI: " + IntToStr(int(ini));
	strFin = "FIN: " + IntToStr(int(fin));
	...
	while (dir != NULL) {
		...
		NodoQ* sigDir = dir->sig;
		TC->Brush->Style = bsClear;
		TC->TextOut(posX + sElem*0.5, posY, dir->elemento);
		TC->TextOut(posX + sElem + sSig*0.5, posY, int(sigDir));
		posY += h;
